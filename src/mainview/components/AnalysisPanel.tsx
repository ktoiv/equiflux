...
function IndicatorCard({
	...
	return (
		<div className="bg-surface-container-lowest p-lg rounded-[16px] soft-breath-shadow border border-transparent hover:border-primary-container transition-colors">
			<span className="block text-label-md text-on-surface-variant mb-2 uppercase tracking-wider">
				{label}
			</span>
	...
	return (
		<div className="flex items-center justify-between px-sm py-1.5 bg-surface-container-lowest rounded-lg">
			...
			<div className="flex gap-lg text-[11px] tabular-nums text-on-surface font-medium">
				{values.map((v, i) => (
					<span key={i} className="w-16 text-right">{fmtLarge(v)}</span>
				))}
			</div>
		</div>
	);
...
function AnalysisPanel({ fundamentals, isLoading, reports, reportsLoading }: AnalysisPanelProps) {
	...
	const latestNetIncome = financials.length > 0 ? num(financials[0], 'netIncome', 'netIncomeFromContinuingOperationNetMinorityInterest') : null;
	...
	return (
		<aside className="w-[360px] h-screen max-h-screen fixed right-0 border-l border-outline-variant bg-surface-container-low flex flex-col">
			...
			<div className="space-y-lg pr-1">
				...
				{isLoading && (
					<p className="text-label-md text-on-surface-variant text-center py-4">Loading...</p>
				)}
			...
			{f && (
				<>
					...
					{showReports && (
						<div className="space-y-lg pr-1">
							{reportsLoading && (
								<p className="text-label-md text-on-surface-variant text-center py-4">Loading reports...</p>
							)}
										...
										<div>
											...
											<div className="space-y-2">
												<div className="flex items-center justify-between px-sm">
													<span className="text-[11px] text-on-surface-variant/60">Metric</span>
													<div className="flex gap-lg text-[11px] text-on-surface-variant/60 tabular-nums">
														{finYears.map((y, i) => <span key={i} className="w-16 text-right">{y}</span>)}
													</div>
												</div>
												<ReportRow label="Revenue" values={financials.slice(0, 3).map((y) => num(y, 'totalRevenue'))} />
												<ReportRow label="Net Income" values={financials.slice(0, 3).map((y) => num(y, 'netIncome', 'netIncomeFromContinuingOperationNetMinorityInterest'))} />
												<ReportRow label="Operating Income" values={financials.slice(0, 3).map((y) => num(y, 'operatingIncome', 'totalOperatingIncomeAsReported'))} />
												<ReportRow label="EBIT" values={financials.slice(0, 3).map((y) => num(y, 'EBIT', 'ebit'))} />
												<ReportRow label="EPS" values={financials.slice(0, 3).map((y) => num(y, 'dilutedEPS', 'basicEPS'))} />
											</div>
										</div>
									)}
										...
										<div>
											...
											<div className="space-y-2">
												<div className="flex items-center justify-between px-sm">
													<span className="text-[11px] text-on-surface-variant/60">Metric</span>
													<div className="flex gap-lg text-[11px] text-on-surface-variant/60 tabular-nums">
														{balYears.map((y, i) => <span key={i} className="w-16 text-right">{y}</span>)}
													</div>
												</div>
												<ReportRow label="Total Assets" values={balanceSheets.slice(0, 3).map((y) => num(y, 'totalAssets'))} />
												<ReportRow label="Total Liabilities" values={balanceSheets.slice(0, 3).map((y) => num(y, 'totalLiabilities'))} />
												<ReportRow label="Total Equity" values={balanceSheets.slice(0, 3).map((y) => num(y, 'totalEquity'))} />
												<ReportRow label="Cash" values={balanceSheets.slice(0, 3).map((y) => num(y, 'cash', 'cashAndCashEquivalents'))} />
												<ReportRow label="Long-term Debt" values={balanceSheets.slice(0, 3).map((y) => num(y, 'longTermDebt', 'totalDebt'))} />
											</div>
										</div>
									)}
										...
										<div>
											...
											<div className="space-y-2">
												<div className="flex items-center justify-between px-sm">
													<span className="text-[11px] text-on-surface-variant/60">Metric</span>
													<div className="flex gap-lg text-[11px] text-on-surface-variant/60 tabular-nums">
														{cfYears.map((y, i) => <span key={i} className="w-16 text-right">{y}</span>)}
													</div>
												</div>
												<ReportRow label="Operating CF" values={cashFlows.slice(0, 3).map((y) => num(y, 'operatingCashFlow', 'cashFromOperatingActivities'))} />
												<ReportRow label="Capital Exp." values={cashFlows.slice(0, 3).map((y) => num(y, 'capitalExpenditure', 'capitalExpenditures', 'purchaseOfPropertyPlantAndEquipment'))} />
												<ReportRow label="Free Cash Flow" values={cashFlows.slice(0, 3).map((y) => num(y, 'freeCashFlow'))} />
											</div>
										</div>
									)}
								</>
			...
			{f?.description && (
				<div className="bg-primary-fixed p-lg rounded-[16px] border border-primary-container">
					<p className="text-body-sm text-on-primary-fixed-variant leading-relaxed">
						<strong>{f.sector ?? 'Company'} Overview:</strong>{' '}
